...
| `provider` / `model` | 各自独立覆盖主请求路由的字段；省略的字段继承会话日志中最近的主请求路由 |
| `acceptKey` | 将显示的建议采纳到草稿的输入框快捷键（`Tab`、`Alt+Slash`、`Ctrl+Enter` 等）；默认为 `Tab` |

同时省略 `provider` 与 `model` 时，会继承当前已记录主请求的确切路由；也可以单独或成对设置二者，覆盖主请求路由的对应字段。`acceptKey` 会随每条建议事件下发，由 Web 输入框的幽灵文本读取。
